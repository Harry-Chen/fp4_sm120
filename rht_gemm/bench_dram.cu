...
int main(int argc, char **argv) {
    ...
    // Allocate a large pool so that each iteration reads from a different
    // buffer, defeating L2 caching of A. Pool = iters copies of A.
    // Each copy is M*N*2 bytes.
    size_t a_bytes = (size_t)M * N * sizeof(__nv_bfloat16);
    int pool = iters + 5;
    __nv_bfloat16 **d_A_pool = new __nv_bfloat16*[pool];
    for (int i = 0; i < pool; i++)
        CUDA_CHECK(cudaMalloc(&d_A_pool[i], a_bytes));
    ...
    for (int i = 0; i < pool; i++)
        CUDA_CHECK(cudaMemcpy(d_A_pool[i], h_A.data(), a_bytes, cudaMemcpyHostToDevice));
    ...
    // warmup (5 calls, use last 5 pool entries)
    for (int i = 0; i < 5; i++)
        rht_gemm_sm120::rht_gemm_ntt_w_sfc<__nv_bfloat16, __nv_bfloat16, uint8_t, uint8_t, false, false>(
            M, N, d_A_pool[pool - 5 + i], d_B, d_C, d_SFC, d_global_amax, d_rng_state, sm_count, 0);
    ...
    for (int i = 0; i < iters; i++)
        rht_gemm_sm120::rht_gemm_ntt_w_sfc<__nv_bfloat16, __nv_bfloat16, uint8_t, uint8_t, false, false>(
            M, N, d_A_pool[i], d_B, d_C, d_SFC, d_global_amax, d_rng_state, sm_count, 0);
    ...
    printf("RHT-DRAM: M=%d N=%d iters=%d | %.4f ms | %.1f GB/s | %.1f GFLOPS\n",
           M, N, iters, ms_per, gbps, gflops);

    CUDA_CHECK(cudaEventDestroy(start));
    CUDA_CHECK(cudaEventDestroy(stop));
    for (int i = 0; i < pool; i++) cudaFree(d_A_pool[i]);
    cudaFree(d_B); cudaFree(d_C); cudaFree(d_SFC);
